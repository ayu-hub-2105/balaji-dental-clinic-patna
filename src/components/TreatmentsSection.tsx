...
export const TreatmentsSection: React.FC<TreatmentsSectionProps> = ({ onBookTreatment, onOpenEmergency }) => {
  ...
  // The 4 core featured treatments requested by the user
  const featuredIds = [
    'invisalign-aligners',      // 1. Invisalign
    'orthodontic-braces',       // 2. Orthodontic Treatment (Braces)
    'dental-implants',          // 3. Dental Implants
    'laser-teeth-whitening'     // 4. Laser Teeth Whitening
  ];
  ...
  const displayedTreatments = showAllServices
    ? (activeCategory === 'All'
        ? treatments
    ...
    <section id="treatments" className="py-16 md:py-24 bg-slate-50/60 border-b border-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-10 space-y-3">
          ...
          {/* Primary View Toggle & Emergency Action Bar */}
          <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
            <button
              ...
              id="view-all-services-toggle-btn"
              className={`px-5 py-2 rounded-full text-xs sm:text-sm font-bold transition-all cursor-pointer shadow-xs flex items-center gap-2 ${
                showAllServices
                  ? 'bg-slate-900 text-white hover:bg-slate-800 ring-2 ring-slate-900/10'
                  : 'bg-sky-600 hover:bg-sky-700 text-white shadow-sky-600/20 shadow-md'
              }`}
            >
              <span>{showAllServices ? 'Show Top 4 Treatments' : 'View All Services'}</span>
              <ArrowRight className={`w-4 h-4 transition-transform duration-200 ${showAllServices ? 'rotate-90' : ''}`} />
            </button>
            ...
            {onOpenEmergency && (
              <button
                ...
                className="px-4 py-2 rounded-full text-xs sm:text-sm font-extrabold bg-rose-600 hover:bg-rose-700 active:scale-95 text-white transition-all cursor-pointer shadow-sm shadow-rose-600/30 border border-rose-500 flex items-center gap-1.5"
              >
          ...
          {/* Category Filter Pills - displayed when all services are shown */}
          {showAllServices && (
            <div className="flex flex-wrap items-center justify-center gap-2 pt-2 animate-fadeIn">
              {categories.map((cat) => (
                <button
                  key={cat}
      ...
      {selectedTreatment && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl border border-slate-200 relative max-h-[90vh] overflow-y-auto text-left animate-fadeIn">
            ...
            {/* Treatment Photo in Modal */}
            <div className="relative h-44 sm:h-52 w-full rounded-2xl overflow-hidden mb-4 border border-slate-200">
              <img
              ...
              <div className="absolute bottom-3 left-3 right-3 text-white">
                <p className="text-xs font-semibold text-sky-200">{selectedTreatment.category} Clinical Specialty</p>
                <h4 className="text-base sm:text-lg font-bold">{selectedTreatment.title}</h4>
              </div>
            </div>
